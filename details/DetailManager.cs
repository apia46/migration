...
public partial class DetailManager : Node
{
    ...
    public const int UNDETAIL_CHUNKS_AROUND_PLAYER = 9;
    ...
    public static void PlayerCrossedChunkBoundary(Vector2I to, Vector2I direction)
	{
        ...
        ProceduralGenerator.Mutex.Lock();
		for (int h = -UNDETAIL_CHUNKS_AROUND_PLAYER; h <= UNDETAIL_CHUNKS_AROUND_PLAYER; h++)
            UnplaceDetails(to + direction * -UNDETAIL_CHUNKS_AROUND_PLAYER + RotateCCW(direction) * h);
    ...
    public static void RemoveDetail<T>(T detail) where T : Node2D, IDetail<T>
    {
        detail.QueueFree();
    ...
    static void PlaceDetails(Vector2I chunk)
    {
        ...
        for (int i = 0; i < 5; i++) {
            ...
            if (!World.SolidTile(tile) && (tileSlopeNormal is null)) continue;
            float rotation;
            if (tileSlopeNormal is float normal) rotation = normal+PI/2;
            else if (World.InteriorTile(tile + new Vector2I(0,-1))) rotation = 0;
            ...
            Vector2 position = (tile + new Vector2(0.5f,0.5f)) * World.CONVERTED_TILE_SIZE;
            foreach (Shelter checkShelter in Shelter.Instances.Values) if (position.DistanceSquaredTo(checkShelter.Position) < 1e6) goto cont;
        ...
        for (int i = 0; i < 5; i++) {
            Vector2I tile = chunk * ProceduralGenerator.CONVERTED_CHUNK_SIZE + new Vector2I(Game.RNG.Range(0, ProceduralGenerator.CONVERTED_CHUNK_SIZE), Game.RNG.Range(0, ProceduralGenerator.CONVERTED_CHUNK_SIZE));
            if (!World.TileCorridorCenter(tile)) continue;
            Vector2 position = (tile + new Vector2(0.5f,0.5f)) * World.CONVERTED_TILE_SIZE;
            foreach (EyeflowerGroup checkGroup in EyeflowerGroup.Instances.Values) if (position.DistanceSquaredTo(checkGroup.Position) < 1e6) goto cont;
            PlaceDetail<EyeflowerGroup>(position);
            cont:;
        }
    ...
    static void UnplaceDetails(Vector2I chunk)
    {
        ...
        UnplaceType<Shelter>();
        UnplaceType<EyeflowerGroup>();
        ProceduralGenerator.ChunkStates[chunk] = ProceduralGenerator.ChunkState.Generated;
    }

    static void CreateCircleColliders()
    {
        static IEnumerable<CircleCollider> GetCreatureColliders<T>() where T : Node2D, ICreature<T> {
            return T.Creatures.Values.Where(c=>c.CollisionRadius != 0 && c.Position.DistanceSquaredTo(World.Player.Position) < Player.OFFSCREEN_RANGE_SQUARED).Select(c=>new CircleCollider(c.Position, c.CollisionRadius));
        }
